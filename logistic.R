...
# Different corrplot 
corrgram(model_d, order = F, upper.panel = panel.cor, main="gumd_binary")
...
#### MODEL 2: Same as MODEL 1 BUT WITHOUT WATER_FLUOR #########################################################################
# water_fluor is not statistically significant at the 95%, so we remove it


# Fitting Logistic Regression to the Training set 
model_2 = glm(formula = gumd_binary ~ age + n.fillings + BMI + Flmins + alcohol + mouthwash + sex + smoke + typ2_diabetes,
                 family = binomial,
                 data = training_set)
...
round(pR2(model_2)[4], 3) # McFadden R_sq = 0.476

#### Predicting the Test set results #####################################################
prob_pred_model2 = predict(model_2, type = "response", newdata = test_set[-11])
...
# Accuracy = (TP + TN) / (TN + TP + FN + FP) - Accurate Positive and Negative Predictions
Accuracy_model2 = round((cm[1, 1] + cm[2, 2]) / (cm[1, 1] + cm[2, 2] + cm[1, 2] + cm[2, 1]), 2) # 0.83
# Precision = TP / Total Predicted Positive (FP + TP) - How precise are the positive predictions 
Precision_model2 = round(cm[2, 2] / (cm[1, 2] + cm[2, 2]), 2) # 0.73
# Recall = TP / Total Actual Positive (TP + FN) - How many of the Actual Positives are predicted accurately as Positives
Recall_model2 = round(cm[2, 2] / (cm[2, 1] + cm[2, 2]), 2) # 0.72
# Recall is extremely important in disease detection. If a sick patient (actual positive)
# is diagnosed as non patient (False Negative), it can have significant impacts to his/her life.

# F1 score = 2 * Precision * Recall / (Precision + Recall) - It is the harmonic mean of Precision and Recall
F1score_model2 = round(2*Precision_model2*Recall_model2 / (Precision_model2 + Recall_model2), 2) # 0.72

# ROC analysis
roc.curve(test_set$gumd_binary, prob_pred_model2) # 0.898

# Likelihood Ratio Test
logLik(model_2) # -266.53
lr.test(logLik(model_2), logLik(model_1)) # -2(LL2 - LL1)

# I can't find another variable to remove at this point. Sex is statistically significant at the 99%.


#### FORWARD SELECTION ##################################################################################
#### MODEL 1: Find the variable with the lowest p-value #########################################################################

# Fitting Logistic Regression to the Training set 
model_11 = glm(formula = gumd_binary ~ age,
              family = binomial,
              data = training_set)
...
# Fitting Logistic Regression to the Training set 
model_12 = glm(formula = gumd_binary ~ n.fillings,
               family = binomial,
               data = training_set)

# See outputs
summary(model_12) # n.fillings more significant than age

# Fitting Logistic Regression to the Training set 
model_13 = glm(formula = gumd_binary ~ BMI,
               family = binomial,
               data = training_set)

# See outputs
summary(model_13) # n.fillings is more significant than BMI

# Fitting Logistic Regression to the Training set 
model_14 = glm(formula = gumd_binary ~ Flmins,
               family = binomial,
               data = training_set)

# See outputs
summary(model_14) # n.fillings is more significant than Flmins


# Fitting Logistic Regression to the Training set 
model_15 = glm(formula = gumd_binary ~ Flmins,
               family = binomial,
               data = training_set)
